...
from .errors import GoogleError, InAppPyError, IAP_ValidationError
...
class GooglePlayValidator:
    ...
    def __init__(self, bundle_id: str, api_key: str, default_valid_purchase_state: int = 0) -> None:
        ...
        """
        if not bundle_id:
            raise IAP_ValidationError("bundle_id cannot be empty.")

        elif not api_key:
            raise IAP_ValidationError("api_key cannot be empty.")
    ...
    def _validate_signature(self, receipt: str, signature: str) -> bool:
        try:
            ...
            validate = rsa.verify(receipt.encode(), sig, self.public_key)
            return validate
