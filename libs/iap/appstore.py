...
api_result_ok = 0
api_result_errors = {
    21000: IAP_ValidationError("Bad json"),
    21002: IAP_ValidationError("Bad data"),
    21003: IAP_ValidationError("Receipt authentication"),
    21004: IAP_ValidationError("Shared secret mismatch"),
    21005: IAP_ValidationError("Server is unavailable"),
    21006: IAP_ValidationError("Subscription has expired"),
    # two following errors can use auto_retry_wrong_env_request.
    21007: IAP_ValidationError("Sandbox receipt was sent to the production env"),
    21008: IAP_ValidationError("Production receipt was sent to the sandbox env"),
    21009: IAP_ValidationError("Internal data access error"),
    21010: IAP_ValidationError("The user account cannot be found or has been deleted"),
}
...
class AppStoreValidator:
    ...
    def post_json(self, request_json: dict) -> dict:
        ...
        try:
            ...
            raise IAP_ValidationError("HTTP error")
    ...
    def validate(
        ...
        if status != api_result_ok:
            error = api_result_errors.get(status, IAP_ValidationError("Unknown API status"))
